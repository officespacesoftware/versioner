...
${autoGeneratedFooter()}`;
   ...
   * - Production carrying commits the branch does not have is silent. Their files merge
   *   in production's favour with no conflict at all, so production keeps changes the
   *   shipped artifact does not have — and the next branch cut from it inherits them.
   *   A hotfix landing after the release branch was cut is the ordinary way in.
   *
    ...
    if (behind.length > 0) {
      ...
      warnings.push(
        `${productionBranch} holds ${behind.length} commit(s) that ${branch} does not, ` +
          `touching ${files.length} file(s)${fileList}. Merging ${branch} into ` +
          `${productionBranch} keeps ${productionBranch}'s side of those files without ` +
          `reporting a conflict, so ${productionBranch} would end up with changes the ` +
          `${branch} artifact does not carry. Merge ${productionBranch} into ${branch} ` +
