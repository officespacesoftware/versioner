...
${autoGeneratedFooter()}`;
  ...
  async planDownmergeHotfixToDevelop(version?: string): Promise<ChangePlan> {
    ...
    return this.buildMergeBranchDownmergePlan({
      ...
      closesTrackingPrFrom: cleanName,
    });
  }
  ...
  private async buildMergeBranchDownmergePlan(opts: {
    ...
    /**
     * A branch whose open pull request into the base this action supersedes and
     * closes, so the base is not left with two open pull requests for one change.
     */
    closesTrackingPrFrom?: string;
  }): Promise<ChangePlan> {
    ...
    if (opts.closesTrackingPrFrom) {
      const tracking = await this.gitFlowManager.findOpenPullRequest(
        opts.closesTrackingPrFrom,
        baseBranch
      );
      if (tracking) {
        mutations.push({
          ...
          operation: "delete",
          summary:
            `close PR #${tracking.number} ` +
            `(${opts.closesTrackingPrFrom} → ${baseBranch}), superseded by the ` +
            `pull request above`,
        });
      }
    }
