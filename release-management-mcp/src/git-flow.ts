...
🤖 Auto-generated by [Release Management MCP](https://github.com/officespacesoftware/versioner/tree/main/release-management-mcp)
      ...
      // Create PR using gh CLI - escape single quotes for shell safety
      const escapedTitle = defaultTitle.replaceAll("'", "\\'");
      const escapedBody = defaultBody.replaceAll("'", "\\'");
      const command = `gh pr create --base ${baseBranch} --head ${branchName} --title '${escapedTitle}' --body '${escapedBody}'`;
