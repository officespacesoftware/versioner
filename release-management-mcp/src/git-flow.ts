...
🤖 Auto-generated by [Release Management MCP](https://github.com/officespacesoftware/versioner/tree/main/release-management-mcp)
    ...
    try {
      ...
      const command = `gh pr create --base ${baseBranch} --head ${branchName} --title '${escapedTitle}' --body-file '${tmpFile}'`;
  ...
  private async updateExistingPullRequest(
    ...
    const { number, body: existingBody, url } = JSON.parse(
      prInfo.stdout.trim()
    );

    // Replace only the watermarked section, preserving user-added content
...
This PR merges the release branch into main after deployment to production.
